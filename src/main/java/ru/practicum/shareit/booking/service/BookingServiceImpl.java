...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public List<BookingResponseDto> getOwnerBookings(Long ownerId, String stringState) {
        ...
        switch (state) {
            case ALL:
                bookings = repository.findByItem_OwnerIdOrderByStartDesc(ownerId);
                break;
            case CURRENT:
                bookings = repository.findByItem_OwnerIdAndStartBeforeAndEndAfterOrderByStartDesc(ownerId, now, now);
                break;
            case PAST:
                bookings = repository.findByItem_OwnerIdAndEndBeforeOrderByStartDesc(ownerId, now);
                break;
            case FUTURE:
                bookings = repository.findByItem_OwnerIdAndStartAfterOrderByStartDesc(ownerId, now);
                break;
            case WAITING:
                bookings = repository.findByItem_OwnerIdAndStatusEqualsOrderByStartDesc(ownerId, StatusBooking.WAITING);
                break;
            case REJECTED:
                bookings = repository.findByItem_OwnerIdAndStatusEqualsOrderByStartDesc(ownerId, StatusBooking.REJECTED);
                break;
            default:
                throw new BadRequestException("Неверный параметр state: " + stringState);
        }
